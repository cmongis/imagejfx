...
/**
 * This service is also a SkinFactory used for creating skin for the
 * InputControl depending on the type of input. The different skins
 * (@InputSkinPlugin) are loaded using SciJava. Using the method
 * <code>canHandle</code> of each plugin, the service detects if a plugin can
 * handle a specific type of input. It requires to keep instances of each plugin
 * all the time. To reduce the memory footprint, a good practice is that each
 * plugin should instanciate graphic element in there <code>init()</code> method
 * and not in the constructor. Thus, the <code>init()</code> is called after
 * dependency injection so if some plugins requires services, they should keep
 * most of their initiation logic in this method.
 *
 * @author Cyril MONGIS
 */
...
public class DefaultInputSkinService extends AbstractService implements InputSkinPluginService {
    ...
    private HashMap<PluginInfo, InputSkinPlugin> skins;
    ...
    private HashMap<PluginInfo, InputSkinPlugin> getSkins() {
        if (skins == null) {
    ...
    public InputSkin createSkin(Input input) {
        ...
        InputSkinPlugin plugin
                = getSkins().values().stream().filter(skin -> skin.canHandle(input.getType())).findFirst().orElse(null);
    ...
    public boolean canCreateSkinFor(Input input) {
        ...
        return getSkins().values().stream().filter(skin -> skin.canHandle(input.getType())).count() > 0;
    }
    ...
    public int getHandledInputs(Module module) {
        ...
        return (int) module.getInputs()
                ...
                .filter(keyName -> {
                    ...
                    logger.info(
                            String.format("[%s] %s : is resolved (%s) / can create (%s) / isRequired (%s) / isNull (%s) == Final Decision ==> %s", module.getDelegateObject().getClass().getSimpleName(), keyName, isResolved, canCreateSkinFor, input.isRequired(), isNull, finalDecision));
