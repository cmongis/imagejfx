...
/**
 * bUnwarpJ plugin for ImageJ(C). Copyright (C) 2005-2010 Ignacio
 ...
 * Class to create the Input/Output dialog to deal with the files to keep the
 * information of bUnwarpJ.
 */
public class IODialogFX extends Window implements ActionListener {
 ...
 /*------------------------------------------------------------------*/
    public IODialogFX() {
        ...
        logger.info("Creating IODialog model");
        logger.info("IODialog created");
    }
    ...
    public IODialogFX(
        ...
        saveAsButton.setOnMouseClicked(e -> savePoints());
...
//        loadSourceInitialAffineMatrixButton.addActionListener(this);
        ...
        pane.getChildren().add(separation1);
        pane.getChildren().add(loadButton);
        pane.getChildren().add(saveAsButton);
        pane.getChildren().add(show_PointsButton);
        pane.getChildren().add(loadTransfButton);
        pane.getChildren().add(loadRawTransfButton);
        pane.getChildren().add(compareOppositeTransfButton);
        pane.getChildren().add(compareElasticRawTransfButton);
        pane.getChildren().add(compareRawButton);
        pane.getChildren().add(convertToRawButton);
        pane.getChildren().add(convertToElasticButton);
        pane.getChildren().add(composeElasticButton);
        pane.getChildren().add(composeRawButton);
        pane.getChildren().add(composeRawElasticButton);
        pane.getChildren().add(invertRawButton);
        pane.getChildren().add(evaluateSimilarityButton);
        pane.getChildren().add(adaptCoeffsButton);
        pane.getChildren().add(loadSourceMaskButton);
        pane.getChildren().add(loadSourceInitialAffineMatrixButton);
        pane.getChildren().add(separation2);
...
//	} /* end getInsets */
 ...
 /*------------------------------------------------------------------*/
    ...
    /*------------------------------------------------------------------*/
    /**
     * Adapt the transformation coefficients to a new image size. It asks the
     * user to introduce the image factor between the previous and the new image
     * size. This factor can be a double to represent image size reductions.
     * Powers of two (positive or negative) expected.
     */
    ...
    /*------------------------------------------------------------------*/
    /**
     * Compare two opposite transformations (direct and inverse) represented by
     * B-splines through the warping index.
     */
    ...
    /*------------------------------------------------------------------*/
    /**
     * Compose two transformations represented by elastic B-splines into a raw
     * mapping table (saved as usual).
     */
    ...
    /*------------------------------------------------------------------*/
    /**
     * Compose a raw transformation with an elastic transformation represented
     * by elastic B-splines into a raw mapping table (saved as usual).
     */
    ...
    /*------------------------------------------------------------------*/
    /**
     * Compare an elastic B-spline transformation with a random deformation (in
     * raw format) by the warping index.
     */
    ...
    /*------------------------------------------------------------------*/
    /**
     * Calculate the similarity error between the source and target images. The
     * error is calculated pixel by pixel without representing the images by
     * B-splines coefficients. Mask are taken into account.
     */
