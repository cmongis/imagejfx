...
/**
 * bUnwarpJ plugin for ImageJ and Fiji. Copyright (C) 2005-2009 Ignacio
 * Arganda-Carreras and Jan Kybic
 *
...
/**
 * ==================================================================== Version:
 * November 2nd, 2009 http://biocomp.cnb.csic.es/%7Eiarganda/bUnwarpJ/
 * \===================================================================
 */
/**
 * Old version (UnwarpJ) information: http://bigwww.epfl.ch/thevenaz/UnwarpJ/
 */
import ij.IJ;
import ij.ImagePlus;
...
public class bUnwarpJ_ extends AbstractImageJ1PluginAdapter {
    ...
    private String max_scale_deformation;
    /**
...
//        // Source and target image plus
//        this.sourceImp = this.getInput(sourceDataset);
//        this.targetImp = this.getInput(outputDataset);
//
...
//        bUnwarpJ_.saveTransformation = dialog.getNextBoolean();
    ...
    //------------------------------------------------------------------
    /**
     * Method for images alignment with no graphical interface. This method
     * gives as result a Transformation object that contains all the
     ...
     * @param img_subsamp_fact image subsampling factor (from 0 to 7,
     * representing 2^0=1 to 2^7 = 128)
     * @param min_scale_deformation (0 - Very Coarse, 1 - Coarse, 2 - Fine, 3 -
     * Very Fine)
     * @param max_scale_deformation (0 - Very Coarse, 1 - Coarse, 2 - Fine, 3 -
     * Very Fine, 4 - Super Fine)
    ...
    /**
     * Method for images alignment with no graphical interface. This method
     * gives as result an array of 2 ImagePlus containing the source-target and
     * target-source results respectively. Each of them has three slices: the
     * final deformed image, its target image and its mask.
     *
     * @param targetImp input target image
     * @param sourceImp input source image
     * @param targetMskIP target mask
     ...
     * @param stopThreshold stopping threshold
     *
     * @return resulting ImagePlus array (with source-target and target-source
     * results)
     */
     ...
     * Method for images alignment with no graphical interface. This method
     * gives as result an array of 2 ImagePlus containing the source-target and
     * target-source results (only final transformed image).
     *
     * @param targetImp input target image
     * @param sourceImp input source image
     * @param targetMskIP target mask
...
//        }
    ...
    //------------------------------------------------------------------
    /**
     * Method to adapt coefficients to new image size. The factor between the
     * old and the new image size is expected to be a power of 2, positive or
     * negative (8, 4, 2, 0.5, 0.25, etc).
     *
    ...
    //------------------------------------------------------------------
    /**
     * Method to compare two opposite elastic deformations through the warping
     * index.
     *
     * @param args program arguments
     */
    ...
    //------------------------------------------------------------------
    /**
     * Method to compare an elastic deformation with a raw deformation through
     * the warping index (both transformations having same direction).
     *
     * @param args program arguments
     */
    ...
    //------------------------------------------------------------------
    /**
     * Method to compare two raw deformations through the warping index (both
    ...
    //------------------------------------------------------------------
    /**
     * Method to transform the source image given an elastic deformation. To be
     * called by the macro language instruction "call":
     ...
     * -elastic_transform : TRANSFORM A SOURCE IMAGE WITH A GIVEN ELASTIC
     * DEFORMATION<br>
    ...
    //------------------------------------------------------------------
    /**
     * Method to transform the source image given an raw deformation. To be
     * called by the macro language instruction "call":
     * <a href="http://rsb.info.nih.gov/ij/developer/macro/functions.html#call">
     * http://rsb.info.nih.gov/ij/developer/macro/functions.html#call</a>
     * <p>
     * It calls the main command line method with the following option:<br>
     * -raw_transform : TRANSFORM A SOURCE IMAGE WITH A GIVEN RAW
     * DEFORMATION<br>
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     ...
     * <p>
     * It calls the main command line method with the following option:<br>
     * -compare_elastic : COMPARE 2 OPPOSITE ELASTIC DEFORMATIONS (BY WARPING
     * INDEX)<br>
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     ...
     * <p>
     * It calls the main command line method with the following option:<br>
     * -compare_elastic_raw : COMPARE AN ELASTIC DEFORMATION WITH A RAW
     * DEFORMATION (BY WARPING INDEX)<br>
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     * target_transformation_file : As saved by bUnwarpJ<br>
     * source_transformation_file : As saved by bUnwarpJ<br>
    ...
    //------------------------------------------------------------------
    /**
     * Method to compare two raw transformations by warping index. To be called
     * by the macro language instruction "call":
     * <a href="http://rsb.info.nih.gov/ij/developer/macro/functions.html#call">
     * http://rsb.info.nih.gov/ij/developer/macro/functions.html#call</a>
     * <p>
     * It calls the main command line method with the following option:<br>
    ...
    //------------------------------------------------------------------
    /**
     * Method to convert an elastic deformation into raw format. To be called by
     * the macro language instruction "call":
     * <a href="http://rsb.info.nih.gov/ij/developer/macro/functions.html#call">
     * http://rsb.info.nih.gov/ij/developer/macro/functions.html#call</a>
     * <p>
     * It calls the main command line method with the following option:<br>
     ...
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     * Input Elastic Transformation File : As saved by bUnwarpJ in elastic
     * format<br>
     * Output Raw Transformation File : As saved by bUnwarpJ in raw format<br>
     *
     * @param targetImageName target image file name (with path)
     * @param sourceImageName source image file name (with path)
     * @param inputElasticTransfFileName input transformation file name (with
     * path)
     * @param outputRawTransfFileName output transformation file name (with
     * path)
     */
    ...
    //------------------------------------------------------------------
    /**
     * Method to compose two elastic transformations. To be called by the macro
     ...
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     * Elastic Transformation File 1 : As saved by bUnwarpJ in elastic
     * format<br>
     * Elastic Transformation File 2 : As saved by bUnwarpJ in elastic
     * format<br>
     * Output Raw Transformation File : As saved by bUnwarpJ in raw format<br>
     *
     * @param targetImageName target image file name (with path)
     * @param sourceImageName source image file name (with path)
     * @param inputElasticTransfFileName1 first input transformation file name
     * (with path)
     * @param inputElasticTransfFileName2 second input transformation file name
     * (with path)
     * @param outputRawTransfFileName output transformation file name (with
     * path)
     */
    ...
    //------------------------------------------------------------------
    /**
     * Method to compose two raw transformations. To be called by the macro
     * language instruction "call":
     * <a href="http://rsb.info.nih.gov/ij/developer/macro/functions.html#call">
     * http://rsb.info.nih.gov/ij/developer/macro/functions.html#call</a>
     * <p>
     ...
     * @param targetImageName target image file name (with path)
     * @param sourceImageName source image file name (with path)
     * @param inputRawTransfFileName1 first input transformation file name (with
     * path)
     * @param inputRawTransfFileName2 second input transformation file name
     * (with path)
     * @param outputRawTransfFileName output transformation file name (with
     * path)
     */
    ...
    //------------------------------------------------------------------
    /**
     * Method to compose a raw transformation with an elastic transformation. To
     * be called by the macro language instruction "call":
     * <a href="http://rsb.info.nih.gov/ij/developer/macro/functions.html#call">
     * http://rsb.info.nih.gov/ij/developer/macro/functions.html#call</a>
     * <p>
     * It calls the main command line method with the following option:<br>
     * -compose_raw_elastic : COMPOSE A RAW DEFORMATION WITH AN ELASTIC
     * DEFORMATION<br>
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     ...
     * @param targetImageName target image file name (with path)
     * @param sourceImageName source image file name (with path)
     * @param inputRawTransfFileName input raw transformation file name (with
     * path)
     * @param inputElasticTransfFileName input elastic transformation file name
     * (with path)
     * @param outputRawTransfFileName output transformation file name (with
     * path)
     */
    ...
    //------------------------------------------------------------------
    /**
     * Method to adapt an elastic transformation given a new image size. To be
     * called by the macro language instruction "call":
     * <a href="http://rsb.info.nih.gov/ij/developer/macro/functions.html#call">
     * http://rsb.info.nih.gov/ij/developer/macro/functions.html#call</a>
     * <p>
     * It calls the main command line method with the following option:<br>
     * -adapt_transform : ADAPT AN ELASTIC DEFORMATION GIVEN A NEW IMAGE
     * SIZE<br>
     * target_image : In any image format<br>
     * source_image : In any image format<br>
     * Input Elastic Transformation File : As saved by bUnwarpJ in elastic
     * format<br>
     * Output Elastic Transformation File : As saved by bUnwarpJ in elastic
     * format<br>
     ...
     * @param inputElasticTransfFileName input elastic transformation file name
     * (with path)
     * @param outputElasticTransfFileName output elastic transformation file
     * name (with path)
