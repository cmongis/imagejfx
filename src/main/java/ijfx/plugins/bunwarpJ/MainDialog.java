...
/**
 * Class to create the dialog for bUnwarpJ. It extends GenericDialog to allow
 * the user interacting with the other windows.
 */
public class MainDialog {
 ...
 /*....................................................................
     ...
     */
    private int max_scale_deformation = 4;
    /**
 ...
 /*------------------------------------------------------------------*/
     ...
     * @param maxImageSubsamplingFactor subsampling factor at highest resolution
     * level
...
//			if(this.modeChoice == MainDialog.MONO_MODE)				
//				this.consistencyWeightTextField.setEnabled(false);
//			else
...
//		{
//			final int new_min_scale_deformation = this.minScaleChoice;
...
//		{
//			final int new_max_scale_deformation = this.maxScaleChoice;
...
//        // Restart the computation of the model
        ...
        if (ph == sourcePh) {
            ...
            for (int i = 0; i < Ydim; i++) {
                ...
                for (int j = 0; j < Xdim; j++, ij++) {
                    if (sourceMsk.getValue(j, i)) //fp.putPixelValue(j,i,    source_data[ij]);
                    {
            ...
            for (int i = 0; i < Ydim; i++) {
                int i_offset = i * Xdim;
                for (int j = 0; j < Xdim; j++, ij++) {
                    if (targetMsk.getValue(j, i)) //fp.putPixelValue(j,i,    source_data[ij]);
                    {
                        fp_array[j + i_offset] = (float) source_data[ij];
                    } else //fp.putPixelValue(j,i,0.5*source_data[ij]);
                    {
 ...
 /*------------------------------------------------------------------*/
    ...
    /*------------------------------------------------------------------*/
    /**
     * Ungray image. It restores the original version of the image (without
     * mask).
     *
 ...
 /*------------------------------------------------------------------*/
     ...
     * source image. The resolution pyramid is not started here, but in
     * startPyramids.
     *
     * @param bIsReverse determines the transformation direction
